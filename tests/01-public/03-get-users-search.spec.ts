...
// ============================================================================
// Endpoint : GET /users/search?q=Emily
// Name     : Emily
// Level    : auditing
// TestTypes: happy-path, negative-testing, contract, security
// ============================================================================

test.describe('Endpoint: GET /users/search?q=Emily', () => {

  test('[CP-GEN-01] GET /users/search?q=Emily - Health Check Reachable', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      expect(response.status(), "Endpoint must respond (no 5xx)").toBeLessThan(500);
  });

  test('[CP-GEN-02] GET /users/search?q=Emily - Latency Baseline < 2000ms', async ({ api }) => {
      const start = Date.now();
      await api.get('/users/search?q=Emily');
      const duration = Date.now() - start;
      expect(duration, "Response must be under 2000ms baseline").toBeLessThan(2000);
  });

  test('[CP-GEN-03] GET /users/search?q=Emily - Response Time SLA', async ({ api }) => {
      const start = Date.now();
      await api.get('/users/search?q=Emily');
      const duration = Date.now() - start;
  ...
  test('[CP-GEN-07] GET /users/search?q=Emily - Content-Type Match', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
  ...
  test('[CP-GEN-04] GET /users/search?q=Emily - X-Content-Type-Options Header', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      const header = response.headers()['x-content-type-options'];
      expect(header, 'x-content-type-options must be nosniff').toBe('nosniff');
  });

  test('[CP-GEN-06] GET /users/search?q=Emily - Strict-Transport-Security Header', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
  ...
  test('[CP-GEN-05] GET /users/search?q=Emily - X-Frame-Options Header', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      const header = response.headers()['x-frame-options'];
      expect(header, 'x-frame-options must be DENY or SAMEORIGIN').toMatch(/^(DENY|SAMEORIGIN)$/i);
  });

  test('[CP-GEN-08] GET /users/search?q=Emily - Content-Security-Policy Header', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
  ...
  test('[CP-GEN-10] GET /users/search?q=Emily - No Authentication Required', async ({ request }) => {
      // Public API — endpoint must be accessible without credentials (no 401/403)
      const response = await request.get((process.env.BASE_URL || '').replace(/\/$/, '') + (('/users/search?q=Emily').startsWith('/') ? ('/users/search?q=Emily') : '/' + ('/users/search?q=Emily')));
  ...
  test('[CP-MET-GET-01] GET /users/search?q=Emily - Successful Retrieval', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      expect(response.status()).toBe(200);
  });

  test('[CP-MET-GET-02] GET /users/search?q=Emily - Collection Returns Array', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.ok()) {
  ...
  test('[CP-MET-GEN-01] GET /users/search?q=Emily - Wrong Method Does Not Cause 5xx', async ({ api }) => {
      const response = await api.post('/users/search?q=Emily');
      expect([201, 400, 401, 403, 404, 405, 409, 422, 429], 'POST on this endpoint must return a non-5xx status').toContain(response.status());
  });

  test('[CP-COD-2XX] GET /users/search?q=Emily - Returns 200', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      const allowed = [200, 201, 204, 429];
      expect(allowed, 'GET /users/search?q=Emily must return a success status code').toContain(response.status());
  });

  test('[CP-RES-01] GET /users/search?q=Emily - Root Strict Schema Match', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
  ...
  test('[CP-RES-04] GET /users/search?q=Emily - Collection Always Returns Array', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.ok()) {
          const body = await response.json();
          expect(
  ...
  test('[CP-RES-05] GET /users/search?q=Emily - Nested Objects Schema Match', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-RES-06] GET /users/search?q=Emily - Array Items Schema Match', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-RES-02] GET /users/search?q=Emily - All Fields Present', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-RES-03] GET /users/search?q=Emily - Atomic Type and Format Assertions', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-RES-08] GET /users/search?q=Emily - Array Cardinality vs Count Fields', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-RES-10] GET /users/search?q=Emily - Array Item IDs Are Unique', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.status() === 429) { return; } // rate-limited: skip schema check
      expect(response.ok()).toBeTruthy();
      const body = await response.json();
  ...
  test('[CP-SEC-02] GET /users/search?q=Emily - No Sensitive Data Exposed', async ({ api }) => {
      const response = await api.get('/users/search?q=Emily');
      if (response.ok()) {
  ...
  test('[CP-SEC-03] GET /users/search?q=Emily - Errors Do Not Expose Internal Details', async ({ api }) => {
      // GET with an invalid path suffix — guaranteed to return a non-2xx error response
      const response = await api.get('/users/search/___error___trigger___');
  ...
  test('[CP-SEC-06] GET /users/search?q=Emily - Injection via Query String Rejected', async ({ api }) => {
  ...
  test('[CP-SEC-01] GET /users/search?q=Emily - Rate Limiting Triggers 429', async ({ api }) => {
      // Send 20 rapid requests to trigger rate limiting
      const requests = Array.from({ length: 20 }, () =>
          api.get('/users/search?q=Emily')
      );
  ...
  test('[CP-AUD-01] GET /users/search?q=Emily - XSS Payloads via Query Not Reflected', async ({ api }) => {
  ...
  test('[CP-AUD-02] GET /users/search?q=Emily - Command Injection via Query Rejected', async ({ api }) => {
  ...
  test('[CP-AUD-03] GET /users/search?q=Emily - Path Traversal via Query Rejected', async ({ api }) => {
  ...
  test('[CP-AUD-04] GET /users/search?q=Emily - Undeclared HTTP Verb Returns 405/403', async ({ api }) => {
      // Send DELETE to an endpoint declared as GET
      const response = await api.delete('/users/search?q=Emily');
  ...
  test('[CP-AUD-05] GET /users/search?q=Emily - Malformed Auth Token Returns 401/403', async ({ api }) => {
  ...
  test('[CP-AUD-06] GET /users/search?q=Emily - JWT alg:none Attack Returns 401/403', async ({ api }) => {
  ...
  test('[CP-AUD-07] GET /users/search?q=Emily - Duplicate Query Params Do Not Cause 500', async ({ api }) => {
      // Send the same query param multiple times with different values
      const basePath = String('/users/search?q=Emily');
