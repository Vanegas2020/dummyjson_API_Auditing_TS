...
test.describe('Endpoint: GET /users', () => {
  ...
  test('[CP-GEN-10] GET /users - No Authentication Required', async ({ request }) => {
      // Public API — endpoint must be accessible without credentials (no 401/403)
      const response = await request.get((process.env.BASE_URL || '').replace(/\/$/, '') + (('/users').startsWith('/') ? ('/users') : '/' + ('/users')));
  ...
  test('[CP-COD-2XX] GET /users - Returns 200', async ({ api }) => {
      ...
      expect(allowed, 'GET /users must return a success status code').toContain(response.status());
  });
